...
class StyleGAN2Loss(Loss):
    def __init__(self, device, G_mapping, G_synthesis, D, image_encoder_kwargs, augment_pipe=None, cond_augment_pipe=None, style_mixing_prob=0.9, r1_gamma=10, pl_batch_shrink=2, pl_decay=0.01, pl_weight=2):
        ...
        self.preprocess = Compose([
            Resize(self.encoder.visual.input_resolution),
        ...
        with misc.ddp_sync(self.G_mapping, sync):
            ...
            if self.style_mixing_prob > 0:
                with torch.autograd.profiler.record_function('style_mixing'):
                    ...
                    ws[:, cutoff:] = self.G_mapping(c, img_emb, skip_w_avg_update=True)[:, cutoff:]
    ...
    def run_D(self, img, c, sync, img_emb=None):
        if self.augment_pipe is not None:
            ...
            # img_emb = self.cond_augment_pipe(img_emb)
            # with torch.no_grad():
    ...
    def accumulate_gradients(self, phase, real_img, real_c, real_emb, gen_c, gen_z, gen_emb, sync, gain):
        ...
        if do_Dmain or do_Dr1:
            ...
            with torch.autograd.profiler.record_function(name + '_forward'):
                ...
                real_emb_tmp = real_emb.detach().requires_grad_(do_Dr1)
                real_logits = self.run_D(real_img_tmp, real_c, sync=sync, img_emb=real_emb_tmp)
